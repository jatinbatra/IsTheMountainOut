    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          {enabled && verified ? (
            <div className="w-8 h-8 rounded bg-[#2d8a4e]/10 flex items-center justify-center">
              <BellRing className="w-4 h-4 text-[#2d8a4e]" aria-hidden="true" />
            </div>
          ) : (
            <div className="w-8 h-8 rounded bg-[var(--ink-deep)] flex items-center justify-center">
      ...
      {platform === "ios-safari-browser" ? (
        <div className="space-y-2 text-sm text-[color:var(--type-3)]">
          <p className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-[var(--ink-deep)] flex items-center justify-center text-[10px] font-mono text-[color:var(--type-4)]">1</span>
            Tap <Share className="w-3.5 h-3.5 inline text-[color:var(--type-3)]" /> Share
          </p>
          <p className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-[var(--ink-deep)] flex items-center justify-center text-[10px] font-mono text-[color:var(--type-4)]">2</span>
            Pick <Plus className="w-3.5 h-3.5 inline text-[color:var(--type-3)]" /> Add to Home Screen
          </p>
          <p className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-[var(--ink-deep)] flex items-center justify-center text-[10px] font-mono text-[color:var(--type-4)]">3</span>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {canEnable && !enabled && (
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium bg-[color:var(--accent)] text-white rounded hover:opacity-90 transition-colors disabled:opacity-40"
              >
                {subscribing ? (
                  <>
            ...
            {enabled && endpoint && (
              <button
                ...
                disabled={testing}
                className={`inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded transition-colors disabled:opacity-40 ${
                  testResult === "sent"
            ...
            {enabled && !endpoint && (
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded border border-gray-200 text-[color:var(--type-1)] hover:border-gray-300 transition-colors disabled:opacity-40"
              >
                Re-subscribe
              </button>
            )}
